...
public class PopPhoneCallAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.pop_phone_call_item, parent, false);
        ...
        String phone = info.get("yx_t_phone");
        String newPhone;
        ...
        if(info.containsKey("yx_c_at")){
            if(info.get("yx_c_at").equals("0")){
                holder.tv_call_time.setText(Util.getDataTime(info.get("yx_at")));
            }else{
                holder.tv_call_time.setText(Util.getDataTime(info.get("yx_c_at")));
            }
        }else{
            holder.tv_call_time.setText(Util.getDataTime(info.get("yx_at")));
        }
        ...
        if(info.get("yx_calltype").equals("0")){
            holder.tv_call_state.setText("呼出");
        }else if(info.get("yx_calltype").equals("1")){
            holder.tv_call_state.setText("呼入");
        }else{
            holder.tv_call_state.setText("系统呼叫");
        }

        if(Integer.valueOf(info.get("yx_duration")) == 0){
            holder.tv_call_long.setText("未接通");
            holder.tv_phone.setTextColor(activity.getResources().getColor(R.color.red_f4333c));
        }else{
            holder.tv_call_long.setText(Util.getTime(Integer.valueOf(info.get("yx_duration"))));
        ...
        holder.iv_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                    try {
                        String key = info.get("yx_qn_key");
                        String  reqUrl = Constants.host+"work/call_url?key="+ info.get("yx_qn_key");
                        OkHttpUtils.get(activity, reqUrl, new OkhttpCallback()
                        {
                            ...
                            public Object parseNetworkResponse(JSONObject object) throws
                                    Exception {
                                if(object != null){
                                    String downUrl = object.getString("qiniuurl");
                                    new AsyncTask<String,Integer,String>(){


                                        @Override
                                        protected String doInBackground(String... strings) {
                                            file = DownLoadUtil.downLoad(downUrl,key);
                                            return file;
                                        }
...
//                    txtInfo.setText("播放已经停止");
   ...
   public void videoVoiceControll(){
    ...
    public PopPhoneCallAdapter(Activity activity, List<Map<String,String>> infos) {
